...
export function useDrivers({ showNotification, loadData, vehicles }: UseDriversProps) {
  ...
  const handleAddDriver = useCallback(async (e: React.FormEvent) => {
    ...
    const payload = { 
      ...driverForm, 
      dob: driverForm.dob || null, license_expiry: driverForm.license_expiry || null, 
      vehicle_insp_expiry: driverForm.vehicle_insp_expiry || null, hire_date: driverForm.hire_date || null, 
      ...
      has_adr: Boolean(driverForm.has_adr), has_own_vehicle: Boolean(driverForm.has_own_vehicle) 
    }
  ...
  const handleSaveEditDriver = useCallback(async (e: React.FormEvent) => {
    ...
    const payload = { 
      ...editDriverForm, 
      dob: editDriverForm.dob || null, license_expiry: editDriverForm.license_expiry || null, 
      vehicle_insp_expiry: editDriverForm.vehicle_insp_expiry || null, hire_date: editDriverForm.hire_date || null, 
      daily_rate: editDriverForm.daily_rate ? parseFloat(editDriverForm.daily_rate as string) : null, 
      has_adr: Boolean(editDriverForm.has_adr), has_own_vehicle: Boolean(editDriverForm.has_own_vehicle) 
    }
